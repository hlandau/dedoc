...
local enforceListType = function (cmd)
  if cmd ~= "enumerate" and cmd ~= "itemize" and cmd ~= "description" then
    SU.error("Only 'enumerate', 'itemize', 'description' or 'item' are accepted in lists, found '"..cmd.."'")
  end
end
...
function package:doItem (options, content)
  ...
  if not enumStyle.display and not enumStyle.bullet then
    SILE.call("font", { weight = 800 }, function ()
      SILE.process(content.options)
      SILE.call("quad")
    end)
  end
...
function package.doNestedList (_, listType, options, content)
  ...
  if not SILE.typesetter:vmode() then
    ...
    if not((SILE.settings:get("lists.current.itemize.depth")
        + SILE.settings:get("lists.current.enumerate.depth")
        + SILE.settings:get("lists.current.description.depth")) > 0)
    then
...
function package.declareSettings (_)
  ...
  SILE.settings:declare({
    parameter = "lists.current.description.depth",
    type = "integer",
    default = 0,
    help = "Current description depth (nesting) - internal"
  })
  ...
  SILE.settings:declare({
    parameter = "lists.description.leftmargin",
    type = "measurement",
    default = SILE.measurement("1.5em"),
    help = "Left margin (indentation) for bullet lists (description)"
  })
...
function package:registerCommands ()
  ...
  self:registerCommand("description", function (options, content)
    self:doNestedList("description", options, content)
  end)
