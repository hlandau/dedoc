...
method_txt-roff-mdoc-xsl1_all: $(METHOD_TXT_ROFF_MDOC_XSL1_NONPAGINATED_TARGETS_EXPR) $(METHOD_TXT_ROFF_MDOC_XSL1_PAGINATED_TARGETS_EXPR) $(METHOD_PS_ROFF_MDOC_XSL1_TARGETS_EXPR) $(METHOD_PDF_PS_ROFF_MDOC_XSL1_TARGETS_EXPR) $(METHOD_XHTML_ROFF_MDOC_XSL1_TARGETS_EXPR)
	$(Q)true

$(METHOD_TXT_ROFF_MDOC_XSL1_NONPAGINATED_OUT_DIR)/%.txt: $(METHOD_MDOC_XSL1_OUT_DIR)/%
	$(call INFO,GROFF,$@)
	$(Q)mkdir -p "$(shell dirname "$@")"
	$(Q)$(GROFF) -mandoc -Tutf8 -dpaper=a4 < "$<" > "$@.tmp"
	$(Q)mv "$@.tmp" "$@"

$(METHOD_TXT_ROFF_MDOC_XSL1_PAGINATED_OUT_DIR)/%.txt: $(METHOD_MDOC_XSL1_OUT_DIR)/%
	$(call INFO,GROFF,$@)
	$(Q)mkdir -p "$(shell dirname "$@")"
