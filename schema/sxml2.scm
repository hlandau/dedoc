...
(define (nsctx-lookup nsctx prefix)
  (if (eq? prefix 'xml) (list 'xml "http://www.w3.org/XML/1998/namespace" 'xml)
    (assoc prefix nsctx)))
    ...
    (sxml-with-top-element tree
      (sxml-with-annotation body '*NAMESPACES*
        (hash-fold (lambda (k v prior)
          (if (eq? v 'undeclared)
            (let ((new-prefix (if (eq? k 'http://www.w3.org/XML/1998/namespace) 'xml
                                  (string->symbol (string-append "z" (number->string (next-count)))))))
